...
function si_captcha_get_options() {
  ...
  $si_captcha_option_defaults = array(
	 ...
	 'edd_checkout' => 'true',    // enable on EasyDigitalDownloads checkout form
...
// this function adds the captcha to the easy-digital-downloads checkout form // bumbum
function si_captcha_edd_checkout_form() {
    global $si_captcha_opt;
...
function si_captcha_edd_checkout_post2() {
   ...
   if (isset($_POST['action']) && $_POST['action'] == 'edd_process_checkout' ) {
     if ($si_captcha_opt['edd_checkout'] == 'true') { 
       $validate_result = $si_captcha->si_captcha_validate_code('checkout', 'unlink');
       if($validate_result != 'valid') {
              edd_set_error( 'captcha_error', $validate_result );
       }  else {
	      $si_captcha_checkout_validated = true;
